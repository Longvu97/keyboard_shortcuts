...
router.post('', validator.body(create), async (req, res) => {
  try {
    ...
    console.log(err);
    res.send(jsonError(errors.CREATE_KEYBOARD_SHORTCUT_FAILED));
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    ...
    console.log(err);
    res.send(jsonError(errors.KEYBOARD_SHORTCUT_NOT_FOUND));
  }
});
...
router.patch('/:id', validator.body(update), async (req, res) => {
  try {
    ...
    console.log(err);
    res.send(jsonError(errors.UPDATE_KEYBOARD_SHORTCUT_FAILED));
  }
});
